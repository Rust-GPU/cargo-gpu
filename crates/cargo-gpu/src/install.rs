...
{version_spec}
    ...
    fn update_spec_files(
        ...
        if !skip_rebuild {
            let types = dummy_metadata.find_package("rustc_codegen_spirv-types")?;
            let target_specs_src = types
                .manifest_path
                .as_std_path()
                .parent()
                .context("parent of rustc_codegen_spirv-types Cargo.toml")?
                .join("target-specs");
            if target_specs_src.is_dir() {
                if source.is_path() {
                    // skip copy
