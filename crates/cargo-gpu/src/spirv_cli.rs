...
impl SpirvCli {
    ...
    fn ensure_shader_rust_version_doesnt_conflict_with_any_cargo_locks(
        ...
        // Note how we don't report it if we change the `Cargo.lock` here, because we're actually
        // setting it to the value it should be. Therefore, the value that accords to the shader's
        // Rust toolchain.
        Self::handle_conflicting_cargo_lock_v4(
            shader_crate_path,
            is_force_overwrite_lockfiles_v4_to_v3,
    ...
    /// When Rust < 1.83.0 is being used an error will occur if it tries to parse an `Cargo.lock`
    /// files that use lockfile manifest version 4. Here we check for that.
    ...
    fn handle_v3v4_conflict(
        ...
        Self::replace_carego_lock_manifest_version(offending_cargo_lock, "4", "3")?;
    ...
    /// were.
    pub fn revert_cargo_lock_manifest_versions(
        offending_cargo_locks: Vec<std::path::PathBuf>,
    ) -> anyhow::Result<()> {
        for offending_cargo_lock in offending_cargo_locks {
            log::debug!("Reverting: {}", offending_cargo_lock.display());
            Self::replace_carego_lock_manifest_version(&offending_cargo_lock, "3", "4")?;
        }
    ...
    fn replace_carego_lock_manifest_version(
        offending_cargo_lock: &std::path::Path,
        from_version: &str,
        to_version: &str,
    ) -> anyhow::Result<()> {
