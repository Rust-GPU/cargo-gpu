...
pub fn query_metadata(crate_path: &Path) -> Result<Metadata, QueryMetadataError> {
    ...
    let metadata = MetadataCommand::new()
        .current_dir(&crate_path.canonicalize()?)
        .exec()?;
    Ok(metadata)
}
...
impl MetadataExt for Metadata {
    ...
    fn find_package(&self, name: &str) -> Result<&Package, MissingPackageError> {
        let Some(package) = self
            ...
            return Err(MissingPackageError::new(name.into(), workspace_root));
        };
...
impl MissingPackageError {
    ...
    const fn new(crate_name: String, workspace_root: Utf8PathBuf) -> Self {
        Self {
            crate_name,
            workspace_root,
        }
